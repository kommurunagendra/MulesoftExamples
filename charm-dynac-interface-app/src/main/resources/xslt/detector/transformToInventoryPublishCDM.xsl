<xsl:stylesheet version="3.0"
	...
	<xsl:template name="detector_si_inventory_publish" match="/tmdd:detectorInventoryUpdateMsg">
		<detector_si_inventory_publish xmlns="http://detector.si.cdm.charm.cgi.com"
			version="1.0.0">
			<meta>
				<message_id>
					<uuid>
			...
			<inventory_config>
				...
				<!-- populate Fep if parent-device-id is empty and parent-device-id2 
					is empty -->
				<xsl:for-each select="detector-inventory-item">
					<xsl:choose>
						<xsl:when
							test="(normalize-space(detector-station-inventory-header/parent-device-id) = '') and (normalize-space(detector-station-inventory-header/parent-device-id2) = '')">
							<fep_inventory>
								<action>
									<xsl:value-of
								...
								<fep_inventory_attr>
									...
									<xsl:if
										test="not(normalize-space(detector-station-inventory-header/device-description)='')">
										<description>
										  <xsl:value-of
									...
									<xsl:if
										test="not(normalize-space(detector-station-inventory-header/data-owner)='')">
										<owner>
				...
				<!--populate OutStation if parent-device-id is present and parent-device-id2 
					is empty -->
				<xsl:for-each select="detector-inventory-item">
					<xsl:choose>
						<xsl:when
				...
				<!--populate Detector if parent-device-id is present and parent-device-id2 
					is present -->
				<xsl:for-each select="detector-inventory-item">
					<xsl:choose>
						<xsl:when
							test="(normalize-space(detector-station-inventory-header/parent-device-id) != '') and (normalize-space(detector-station-inventory-header/parent-device-id2) = '')">
							<!-- for each detector inventory item -->
							...
							<xsl:for-each select="detector-list/detector">
								...
								<detector_inventory>
									...
									<detector_inventory_attr>
										...
										<xsl:if
											test="not(normalize-space(detector-inventory-header/device-description)='')">
											<description>
										...
										<detector_address>
											...
											<detector_station_nr>
												<xsl:value-of
													select="detector-inventory-header/station-line-number" />
											</detector_station_nr>
											<detector_nr>
												<xsl:value-of
										...
										<msi_relation>
											<xsl:value-of
												select="detector-inventory-header/associated-device-id" />
										</msi_relation>
									</detector_inventory_attr>
