<xsl:stylesheet version="1.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fep="http://prism.cdm.charm.cgi.com">
	<xsl:output method="xml" indent="no" />
	<xsl:template match="/root">
		<prism_control_reply>
			<xsl:apply-templates select="prism_control_reply[1]/meta" />
			<xsl:for-each
				select="prism_control_reply/prism_state_address_attr_reply">
				<prism_state_address_attr_reply>
					<prism_id>
						<line_id>
							<xsl:value-of select="prism_id/line_id" />
						</line_id>
						<position_id>
							<xsl:value-of select="prism_id/position_id" />
						</position_id>
						<xsl:if test="reference_road_code != ''">
							<reference_road_code>
								<xsl:value-of select="prism_id/reference_road_code" />
							</reference_road_code>
						</xsl:if>
						<xsl:if test="reference_km != ''">
							<reference_km>
								<xsl:value-of select="prism_id/reference_km" />
							</reference_km>
						</xsl:if>
					</prism_id>
					<change_state_reply>
	...
	<xsl:template match="prism_control_reply[1]/meta">
		<meta>
			<message_id>
				<uuid>
